...
export function appearance(p){const g=p.gear;return {cls:p.cls,weapon:(g.weapon||g.offhand)?.weaponType||null,offhand:g.weapon?g.offhand?.weaponType||null:null,offhandTier:g.offhand?.rarity||'common',weaponTier:g.weapon?.rarity||'common',head:g.head?({shadow:4,oath:5,ember:6,saint:6}[p.cls]+(g.head.rarity==='epic'?0:0)):null,chest:g.chest?{layer:g.chest.rarity==='common'?9:8,tier:g.chest.rarity}:null,hands:!!g.hands,feet:!!g.feet,relic:!!g.relic};}
