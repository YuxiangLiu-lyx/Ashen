...
export function loadStoryIllustrationV19(id,onReady){
 ...
 img.onerror=()=>{entry.state='failed';entry.onReady=null;};img.src=globalThis.__ASHEN_WEB__.asset(art.src);
...
export function storyIllustrationHTMLV19(art){
 ...
 return `<figure class="story-illustration-v19${entering?' is-new':''}" data-scene-art="${escape(art.id)}"><img src="${escape(globalThis.__ASHEN_WEB__.asset(art.src))}" alt="${escape(art.alt)}" decoding="async" draggable="false"></figure>`;
}
