...
export class ArtBank{
 ...
 async loadWebOriginal(name,url,cols=1,rows=1,key=false){const im=new Image();im.crossOrigin='anonymous';im.src=url;await im.decode();const c=document.createElement('canvas');c.width=im.width;c.height=im.height;const cx=c.getContext('2d',{willReadFrequently:true});cx.drawImage(im,0,0);const data=cx.getImageData(0,0,c.width,c.height);
